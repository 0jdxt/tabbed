...
void *ecalloc(size_t n, size_t size) {
    ...
    if (!(p = calloc(n, size))) die("%s: cannot calloc\n", argv0);
    return p;
}
...
void *erealloc(void *o, size_t size) {
    ...
    if (!(p = realloc(o, size))) die("%s: cannot realloc\n", argv0);
    return p;
}
...
Bool isprotodel(int c) {
    ...
    if (XGetWMProtocols(dpy, clients[c]->win, &protocols, &n)) {
        for (i = 0; !ret && i < n; i++) {
            if (protocols[i] == wmatom[WMDelete]) ret = True;
        }
        XFree(protocols);
    }
...
void manage(Window w) {
    updatenumlockmask();
    {
        ...
        XSelectInput(
            dpy, w, PropertyChangeMask | StructureNotifyMask | EnterWindowMask);
        XSync(dpy, False);
        ...
        for (i = 0; i < LENGTH(keyreleases); i++) {
            if ((code = XKeysymToKeycode(dpy, keyreleases[i].keysym))) {
                for (j = 0; j < LENGTH(modifiers); j++) {
                    XGrabKey(dpy, code, keyreleases[i].mod | modifiers[j], w,
...
void move(const Arg *arg) {
    if (arg->i >= 0 && arg->i < nclients) focus(arg->i);
}
...
int main(int argc, char *argv[]) {
    ...
    if (!(dpy = XOpenDisplay(NULL))) die("%s: cannot open display\n", argv0);
